...
for (const root of ['dist', 'release.json', 'openapi', 'protocols', 'fixtures', 'README.md', 'recipes', 'package.json']) {
  for (const path of files(resolve(root))) {
    ...
    const isPinnedGeneratedContract =
      path.includes('/openapi/') || path.includes('/generated/');
    // The immutable public contract may carry catalog identity values. Those
    // generated bytes are verified by digest and are not a provider client.
    // Hand-written SDK code, docs, recipes, and fixtures remain provider-free.
    if (!isPinnedGeneratedContract && providerSurface.test(source)) {
      violations.push(`provider:${path}`);
    }
