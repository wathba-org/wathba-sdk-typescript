...
2. From current `main`, run **Bootstrap first SDK publish** and type `PUBLISH @wathba/sdk 0.1.0 ONCE`. The workflow accepts only that package/version, fails if it already exists, exposes the token only as `NODE_AUTH_TOKEN` on the publish step, publishes the packed candidate with provenance, cryptographically verifies the retrieved package through npm's signature-and-attestation audit, binds its exact SLSA subject/source facts, and uploads the resulting evidence.
3. On npm, configure the new package's trusted GitHub Actions publisher for organization `wathba-org`, repository `wathba-sdk-typescript`, workflow `release.yml`, environment `npm-production`, and allowed action `npm publish`.
4. Delete the `NPM_TOKEN` GitHub environment secret and revoke the granular token at npm before proceeding.
5. Tag the exact merged `main` commit as `v0.1.0`. The normal release workflow detects and verifies the identical existing registry version without republishing it, then creates the immutable GitHub release evidence. All later versions publish only through the trusted publisher; do not restore a long-lived npm token.
